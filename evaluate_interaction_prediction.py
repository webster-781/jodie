...
parser.add_argument(
    "--embedding_dim", default=128, type=int, help="Number of dimensions"
)
parser.add_argument(
    "--train_proportion",
    default=0.8,
    type=float,
    help="Proportion of training interactions",
)
parser.add_argument(
    "--state_change",
    default=True,
    type=bool,
    help="True if training with state change of users in addition to the next interaction prediction. False otherwise. By default, set to True. MUST BE THE SAME AS THE ONE USED IN TRAINING.",
)
...
wandb_run = wandb.init(project = "fb-graph-proj", group = "jodie", id=args.runid, resume="must")
# SET GPU
...
(
    ...
    train_end_idx_training,
) = load_model(model, optimizer, args, args.epoch)
...
with trange(train_end_idx, test_end_idx) as progress_bar:
    for j in progress_bar:
        ...
        user_embedding_static_input = user_embeddings_static[
            torch.cuda.LongTensor([userid])
        ]
        item_embedding_input = item_embeddings[torch.cuda.LongTensor([itemid])]
        item_embedding_static_input = item_embeddings_static[
            torch.cuda.LongTensor([itemid])
        ]
        feature_tensor = Variable(torch.Tensor(feature).cuda()).unsqueeze(0)
        user_timediffs_tensor = Variable(
            torch.Tensor([user_timediff]).cuda()
        ).unsqueeze(0)
        item_timediffs_tensor = Variable(
            torch.Tensor([item_timediff]).cuda()
        ).unsqueeze(0)
        item_embedding_previous = item_embeddings[
            torch.cuda.LongTensor([itemid_previous])
        ]
        ...
        loss += MSELoss(
            ...
            torch.cat(
                [item_embedding_input, item_embedding_static_input], dim=1
            ).detach(),
        )

        # CALCULATE DISTANCE OF PREDICTED ITEM EMBEDDING TO ALL ITEMS
        euclidean_distances = nn.PairwiseDistance()(
            predicted_item_embedding.repeat(num_items, 1),
            torch.cat([item_embeddings, item_embeddings_static], dim=1),
        ).squeeze(-1)

        # CALCULATE RANK OF THE TRUE ITEM AMONG ALL ITEMS
        true_item_distance = euclidean_distances[itemid]
        euclidean_distances_smaller = (
            (euclidean_distances < true_item_distance).data.cpu().numpy()
        )
        ...
        if args.state_change:
            loss += calculate_state_prediction_loss(
                model, [j], user_embeddings_timeseries, y_true, crossEntropyLoss
            )
            total_loss += loss
...
for i in range(len(metrics)):
    ...
    fw.write(
        ...
        + metrics[i]
        + ": "
        + str(performance_dict["validation"][i])
        + "\n"
    )

wandb_run.log(
    {
        "val_test_loss": total_loss,
        "val_recall": performance_dict["validation"][1],
        "val_mrr": performance_dict["validation"][0],
        "test_recall": performance_dict["test"][1],
        "test_mrr": performance_dict["test"][0],
    }
)
