...
parser.add_argument(
    "--model", default="jodie", help="Model name to save output in file"
)
parser.add_argument(
    "--gpu",
    default=-1,
    type=int,
    help="ID of the gpu to run on. If set to -1 (default), the GPU with most free memory will be chosen.",
)
parser.add_argument(
...
parser.add_argument(
    ...
    default=128,
    type=int,
    help="Number of dimensions of the dynamic embedding",
)
parser.add_argument(
    "--train_proportion",
    default=0.8,
    type=float,
    help="Fraction of interactions (from the beginning) that are used for training.The next 10% are used for validation and the next 10% for testing",
)
parser.add_argument(
    "--state_change",
    default=True,
    type=bool,
    help="True if training with state change of users along with interaction prediction. False otherwise. By default, set to True.",
)
parser.add_argument("--name", default="sample_jodie_run")
...
wandb_run = wandb.init(
    project="fb-graph-proj",
    config={"dataset": args.network, "run_name": args.name},
    group="JODIE",
)
wandb_run.define_metric("train_loss_cumulative", summary="max")
wandb_run.define_metric("train_loss", summary="max")
wandb_run.save("jodie.py")
wandb_run.save("library_models.py")
wandb_run.save("library_data.py")
wandb_run.define_metric("val_test_loss")
wandb_run.define_metric("val_recall")
wandb_run.define_metric("val_mrr")
wandb_run.define_metric("test_recall")
wandb_run.define_metric("test_mrr")
# LOAD DATA
[
    user2id,
    user_sequence_id,
...
initial_user_embedding = nn.Parameter(
    F.normalize(torch.rand(args.embedding_dim).cuda(), dim=0)
)  # the initial user and item embeddings are learned during training as well
initial_item_embedding = nn.Parameter(
    F.normalize(torch.rand(args.embedding_dim).cuda(), dim=0)
)
model.initial_user_embedding = initial_user_embedding
model.initial_item_embedding = initial_item_embedding

user_embeddings = initial_user_embedding.repeat(
    num_users, 1
)  # initialize all users to the same embedding
item_embeddings = initial_item_embedding.repeat(
    num_items, 1
)  # initialize all items to the same embedding
item_embedding_static = Variable(
    torch.eye(num_items).cuda()
)  # one-hot vectors for static embeddings
user_embedding_static = Variable(
    torch.eye(num_users).cuda()
)  # one-hot vectors for static embeddings
...
with trange(args.epochs) as progress_bar1:
    for ep in progress_bar1:
        ...
        user_embeddings_timeseries = Variable(
            torch.Tensor(num_interactions, args.embedding_dim).cuda()
        )
        item_embeddings_timeseries = Variable(
            torch.Tensor(num_interactions, args.embedding_dim).cuda()
        )
        ...
        with trange(train_end_idx) as progress_bar2:
            for j in progress_bar2:
                ...
                if is_first_epoch:
                    ...
                    tbatch_to_insert = (
                        max(lib.tbatchid_user[userid], lib.tbatchid_item[itemid]) + 1
                    )
                    ...
                    lib.current_tbatches_user_timediffs[tbatch_to_insert].append(
                        user_timediff
                    )
                    lib.current_tbatches_item_timediffs[tbatch_to_insert].append(
                        item_timediff
                    )
                    lib.current_tbatches_previous_item[tbatch_to_insert].append(
                        user_previous_itemid_sequence[j]
                    )
                ...
                if timestamp - tbatch_start_time > tbatch_timespan:
                    tbatch_start_time = (
                        timestamp  # RESET START TIME FOR THE NEXT TBATCHES
                    )
                    ...
                    if not is_first_epoch:
                        ...
                        lib.current_tbatches_interactionids = (
                            cached_tbatches_interactionids[timestamp]
                        )
                        lib.current_tbatches_feature = cached_tbatches_feature[
                            timestamp
                        ]
                        lib.current_tbatches_user_timediffs = (
                            cached_tbatches_user_timediffs[timestamp]
                        )
                        lib.current_tbatches_item_timediffs = (
                            cached_tbatches_item_timediffs[timestamp]
                        )
                        lib.current_tbatches_previous_item = (
                            cached_tbatches_previous_item[timestamp]
                        )

                    with trange(len(lib.current_tbatches_user)) as progress_bar3:
                        for i in progress_bar3:
                            progress_bar3.set_description(
                                "Processed %d of %d T-batches "
                                % (i, len(lib.current_tbatches_user))
                            )

                            total_interaction_count += len(
                                lib.current_tbatches_interactionids[i]
                            )

                            # LOAD THE CURRENT TBATCH
                            if is_first_epoch:
                                lib.current_tbatches_user[i] = torch.LongTensor(
                                    lib.current_tbatches_user[i]
                                ).cuda()
                                lib.current_tbatches_item[i] = torch.LongTensor(
                                    lib.current_tbatches_item[i]
                                ).cuda()
                                lib.current_tbatches_interactionids[
                                    i
                                ] = torch.LongTensor(
                                ...
                                lib.current_tbatches_feature[i] = torch.Tensor(
                                    lib.current_tbatches_feature[i]
                                ).cuda()

                                lib.current_tbatches_user_timediffs[i] = torch.Tensor(
                                    lib.current_tbatches_user_timediffs[i]
                                ).cuda()
                                lib.current_tbatches_item_timediffs[i] = torch.Tensor(
                                    lib.current_tbatches_item_timediffs[i]
                                ).cuda()
                                lib.current_tbatches_previous_item[
                                    i
                                ] = torch.LongTensor(
                                    lib.current_tbatches_previous_item[i]
                                ).cuda()

                            tbatch_userids = lib.current_tbatches_user[
                                i
                            ]  # Recall "lib.current_tbatches_user[i]" has unique elements
                            tbatch_itemids = lib.current_tbatches_item[
                                i
                            ]  # Recall "lib.current_tbatches_item[i]" has unique elements
                            tbatch_interactionids = lib.current_tbatches_interactionids[
                                i
                            ]
                            feature_tensor = Variable(
                                lib.current_tbatches_feature[i]
                            )  # Recall "lib.current_tbatches_feature[i]" is list of list, so "feature_tensor" is a 2-d tensor
                            user_timediffs_tensor = Variable(
                                lib.current_tbatches_user_timediffs[i]
                            ).unsqueeze(1)
                            item_timediffs_tensor = Variable(
                                lib.current_tbatches_item_timediffs[i]
                            ).unsqueeze(1)
                            tbatch_itemids_previous = (
                                lib.current_tbatches_previous_item[i]
                            )
                            item_embedding_previous = item_embeddings[
                                tbatch_itemids_previous, :
                            ]
                            ...
                            loss += MSELoss(
                                ...
                                torch.cat(
                                    [
                                        ...
                                        item_embedding_static[tbatch_itemids, :],
                                    ],
                                    dim=1,
                                ).detach(),
                            )
                            ...
                            ] = user_embedding_output
                            item_embeddings_timeseries[
                                tbatch_interactionids, :
                            ] = item_embedding_output

                            # CALCULATE LOSS TO MAINTAIN TEMPORAL SMOOTHNESS
                            loss += MSELoss(
                                item_embedding_output, item_embedding_input.detach()
                            )
                            loss += MSELoss(
                                user_embedding_output, user_embedding_input.detach()
                            )

                            # CALCULATE STATE CHANGE LOSS
                            if args.state_change:
                                loss += calculate_state_prediction_loss(
                                    model,
                                    tbatch_interactionids,
                                    user_embeddings_timeseries,
                                    y_true,
                    ...
                    total_loss_report += total_loss
                    wandb_run.log({"train_loss_cumulative": total_loss})
                    loss.backward()
                    optimizer.step()
                    optimizer.zero_grad()
        ...
        wandb_run.log({"train_loss": total_loss_report})
        ...
        item_embeddings_dystat = torch.cat(
            [item_embeddings, item_embedding_static], dim=1
        )
        user_embeddings_dystat = torch.cat(
            [user_embeddings, user_embedding_static], dim=1
        )
